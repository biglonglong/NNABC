...
class TextDataset(Data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_onehot = self.encoder.transform(
            np.array([self.int2word[word_indice] for word_indice in input_indices]).reshape(-1, 1))

        return torch.tensor(input_onehot, dtype=torch.float32), torch.tensor(target_indices, dtype=torch.long)